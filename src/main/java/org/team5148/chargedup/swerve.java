...
public class SwerveDrive {
	...
	private static double findEncAng(double angle) {
		...
		while (angle <= -180) { angle += 360; } return angle; } @SuppressWarnings("null") /** * Used to get the setpoint for a PID loop on a swerve wheel * @param motor location of wheel frontLeft, rearLeft, frontRight, rearRight * @return the setpoint value for the wheel */ public double getSetpoint(MotorType motor) { switch(motor.value) { case 0: return pidFL.getSetpoint(); case 1: return pidRL.getSetpoint(); case 2: return pidFR.getSetpoint(); case 3: return pidRR.getSetpoint(); default: return (Double) null; } } private double driveWheel(int wheel, double stickAng, double stickMag, double rot) { double rotAng = 0; double rotMag = rot; double transAng = stickAng; double transMag = stickMag; double currentWheelAngle = 0.0; switch (wheel) { case 0: rotAng = rotAngFL; currentWheelAngle = findEncAng(encFL.getDistance()); break; case 1: rotAng = rotAngRL; currentWheelAngle = findEncAng(encRL.getDistance()); break; case 2: rotAng = rotAngFR; currentWheelAngle = findEncAng(encFR.getDistance()); break; case 3: rotAng = rotAngRR; currentWheelAngle = findEncAng(encRR.getDistance()); break; default: break; } double wheelX = Math.cos(transAng/57.2957795) * transMag + Math.cos(rotAng/57.2957795) * rotMag; double wheelY = Math.sin(transAng/57.2957795) * transMag + Math.sin(rotAng/57.2957795) * rotMag; double wheelRot = Math.atan2(wheelY, wheelX) * 57.2957795; double wheelMag = Math.hypot(wheelX, wheelY); if (goToAngle(wheelRot, currentWheelAngle)) { } else { wheelRot += 180; if (wheelRot > 180) {
				wheelRot -= 360;
			}
			wheelMag = -wheelMag;
		}
	...
	private static boolean goToAngle(double wheelRot, double currentWheelAngle) {
		...
		if (Math.abs(wheelRot - currentWheelAngle) < 90) { return true; } return false; } /** * Used to force swerve drive to move the robot on the arc centered x units to the right * and y units in front of the robot. * @param centerX the x coordinate of the point of rotation to the right of the center of the robot * @param centerY the y coordinate of the point of rotation in front of the center of the robot * @param velocity the speed the wheels will move from -1 to 1. Positive is clockwise */ public void driveRadially(double centerX, double centerY, double velocity) { double flAngle, rlAngle, frAngle, rrAngle = 0.0; if (velocity > 0) {
		...
		if (rlAngle < -180) rlAngle += 360;
		if (frAngle < -180) frAngle += 360;
		if (rrAngle < -180) rrAngle += 360; double flMag, rlMag, frMag, rrMag, maxMag = 0.0; frMag = Math.sqrt(Math.pow(length/2.0-centerY, 2) + Math.pow(width/2.0-centerX, 2)); maxMag = frMag; flMag = Math.sqrt(Math.pow(length/2.0-centerY,2) + Math.pow(-width/2.0-centerX, 2)); if (flMag > maxMag) maxMag = flMag;
	...
	private void applyWheelVector(double angle, double magnitude, int wheel) {
		...
		if (angle < -180) { angle += 360; } switch(wheel) { case 0: pidFL.setSetpoint(angle); if (voltageCompensate) { transFL.set(magnitude * maxVoltage); } else { transFL.set(magnitude); } break; case 1: pidRL.setSetpoint(angle); if (voltageCompensate) { transRL.set(magnitude * maxVoltage); } else { transRL.set(magnitude); } break; case 2: pidFR.setSetpoint(angle); if (voltageCompensate) { transFR.set(magnitude * maxVoltage * 1.2); } else { transFR.set(magnitude * 1.2); } break; case 3: pidRR.setSetpoint(angle); if (voltageCompensate) { transRR.set(magnitude * maxVoltage * 1.2); } else { transRR.set(magnitude * 1.2); } break; default: break; } } /** * Used to update move and rotate values for serve drive. Should be * constantly updated unless values are set to 0 * @param stickAngle calculated angle from joystick [stick.getDirectionDegrees()] * @param stickMag calculated magnatude from joystick [stick.getMagnatude()] * @param rot rotation value for the robot * @param angleOffset degrees to offset the stick angle by */ public void drive(double stickAngle, double stickMag, double rot, double angleOffset) { if (Double.isNaN(rot)) { rot = 0.0; } stickAngle = -stickAngle; //start for converting stick angle value to polar coordinate stickAngle+= 90; if (stickAngle > 180) {
			stickAngle -= 360;
		}
